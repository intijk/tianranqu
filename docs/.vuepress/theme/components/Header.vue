...
export default {
  ...
  methods: {
    handleScroll (event) {
      const currentTop=document.getElementsByTagName('html')[0].scrollTop;
      if(currentTop-this.prevTop>0){
        //hide
        this.$el.querySelector('#header').classList.add('h-is-hidden');
      }else{
        this.$el.querySelector('#header').classList.remove('h-is-hidden');
      }
